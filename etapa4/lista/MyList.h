...
public:
	...
	int eraseMatchingElements(const T& element);
	Node<T>*& reverse(Node<T>* &element);
	void reverse();
...
template<class T>
void MyList<T>::destroy(iterator pos){ 
	if(pos == NULL) return;
	if(pos->next == NULL){
		delete pos;
		return;
	}
	this->destroy(pos->next);
}
...
template<class T>
int MyList<T>::eraseMatchingElements(const T& element){
	int removidos = 0;
...
template<class T>
Node<T>*& MyList<T>::reverse(Node<T>* &pos){
	if(pos == NULL) return pos;
	if(pos->next == NULL){
		return pos->next;
	}
	this->reverse(pos->next) = pos;
	return pos->next;
}

template<class T>
void MyList<T>::reverse(){
	this->reverse(dataFirst) = NULL;
